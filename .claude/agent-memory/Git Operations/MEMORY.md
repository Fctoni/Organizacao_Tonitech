...
- Branch: main
- Alterações: schema de localização com parede — campo `box` (int) → `place` codificado (B<n>/W<n>); subcomandos `relocate`/`migrate`; CONTEXT.md (termo Location + Surface/Box/Wall/Place), SKILL.md (aponta p/ `--help`), ADR 0003, Dashboard migrado; 48 testes verdes; notas FNIRSI/Livolo/Dog Food + Sonoff qty. Migrate ainda NÃO rodado (passo seguinte).
