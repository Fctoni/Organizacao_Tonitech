#!/usr/bin/env python3
"""inventory.py — garage inventory engine (invoked via inventory.sh).

The pure data-processing core of the inventory CLI: frontmatter parsing,
validation, TSV reads and JSON writes. `inventory.sh` is a thin shim that sets
VAULT_DIR and execs this module; the CLI contract lives here. Functions are
importable for unit testing — the dispatch only runs under __main__.

Dependencies: python3 stdlib only. See `inventory.sh --help`.
"""
...
try:
    ...
    sys.exit("VAULT_DIR is not set — run via inventory.sh (the shim sets it).")
...
def validate_category(category):
    if category not in CANONICAL_CATEGORIES:
        die(f"category '{category}' is not in the controlled vocabulary "
            ...
            f"and the CANONICAL_CATEGORIES tuple in inventory.py, then retry.")
